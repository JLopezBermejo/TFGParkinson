...
directory <- paste(directory,'/G13/G13_MI_22_10x10.csv',sep='') 
...
G13SOM <- som(dfScaled, grid = somgrid(10,10,"hexagonal"), rlen = 1000,init = redInit)

#representamos la red

purple.fade <- function(n){
  return(rgb(0,0,0.2,alpha=seq(0,1,1/n)))}
plot(G13SOM, type="count", shape = "straight", palette.name = purple.fade)

plot(G13SOM, type="dist.neighbours", shape = "straight")

#Vemos que clusters ha creado la red
distancesSOM <- dist(getCodes(G13SOM,1))
...
plot(G13SOM,type="mapping",bgcol=c("steelblue1","sienna1")[cluster2], shape = "straight")

plot(G13SOM,type="mapping",bgcol=c("steelblue1","sienna1","yellowgreen")[cluster3], shape = "straight")

plot(G13SOM,type="mapping",bgcol=c("steelblue1","sienna1","yellowgreen","red")[cluster4], shape = "straight")
...
for (j in 1:ncol(df)){
  plot(G13SOM,type="property",property=getCodes(G13SOM,1)[,j], palette.name=purple.fade,main=colnames(df)[j],cex=0.5, shape = "straight")
}


#volvemos a juntar las variables de name y status junto a que neurona ha sido asignada dicho registro
aux <- cbind(name,G13SOM$unit.classif)
...
file <- paste(dirname(getwd()),'/GeneratedGroups/G13/G13.csv',sep='')
write.csv(unionClusters, file,row.names=FALSE,)
pesosSanosEnfermos <- getCodes(G13SOM)
file <- paste(dirname(getwd()),'/GeneratedGroups/G13/G13Pesos.csv',sep='')
