#ESTE ENTRENAMIENTO ES IGUAL AL DEL EXPERIMENTO 11 RESPECTO A LOS PARÁMETROS, PERO CON SÓLO 4 CARACTERÍSTICAS
...
directory <- paste(directory,'/G12/G12_SF_4_10x10.csv',sep='') 
...
G12SOM <- som(dfScaled, grid = somgrid(10,10,"hexagonal"), rlen = 1000, init = redInit)

#representamos la red

purple.fade <- function(n){
  return(rgb(0,0,0.2,alpha=seq(0,1,1/n)))}
plot(G12SOM, type="count", shape = "straight", palette.name = purple.fade)

plot(G12SOM, type="dist.neighbours", shape = "straight")

plot(G12SOM, shape = "straight")

#Vemos que clusters ha creado la red
distancesSOM <- dist(getCodes(G12SOM,1))
...
plot(G12SOM,type="mapping",bgcol=c("steelblue1","sienna1")[cluster2], shape = "straight")

plot(G12SOM,type="mapping",bgcol=c("steelblue1","sienna1","yellowgreen")[cluster3], shape = "straight")

plot(G12SOM,type="mapping",bgcol=c("steelblue1","sienna1","yellowgreen","red")[cluster4], shape = "straight")
...
for (j in 1:ncol(df)){
  plot(G12SOM,type="property",property=getCodes(G12SOM,1)[,j], palette.name=purple.fade,main=colnames(df)[j],cex=0.5, shape = "straight")
}

#volvemos a juntar las variables de name y status junto a que neurona ha sido asignada dicho registro
aux <- cbind(name,G12SOM$unit.classif)
...
file <- paste(dirname(getwd()),'/GeneratedGroups/G12/G12.csv',sep='')
write.csv(unionClusters, file,row.names=FALSE,)
pesosSanosEnfermos <- getCodes(G12SOM)
file <- paste(dirname(getwd()),'/GeneratedGroups/G12/G12Pesos.csv',sep='')
